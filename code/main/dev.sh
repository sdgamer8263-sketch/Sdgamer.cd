#!/bin/bash
# ===========================================================
# SDGAMER - OBSIDIAN NEXT GEN (v12.0 - Custom Banner)
# Style: Segmented UI / Glass-Look / SDGAMER Edition
# ===========================================================

# --- 0. PRE-INITIALIZATION ---
# Hostname detection updated for SDGAMER environment
...
# --- REPO CONFIG ---
# Centralized SDGAMER repository path
...
render_ui() {
    ...
    # --- TOP STATUS PILLS ---
    echo -e " ${B_BLUE}${NC}${BG_SHADE}${W}   SD-HOST: $CURRENT_HOST ${NC}${B_BLUE}${NC}  ${B_PURPLE}${NC}${BG_SHADE}${W}   $UPT ${NC}${B_PURPLE}${NC}  ${B_GREEN}${NC}${BG_SHADE}${W}   $DISK ${NC}${B_GREEN}${NC}"
    echo -e ""

    # --- SDGAMER LARGE BANNER (GRADIENT EFFECT) ---
    echo -e "${B_CYAN} ██████╗██████╗  ██████╗  █████╗ ███╗   ███╗███████╗██████╗ ${NC}"
    echo -e "${B_CYAN}██╔════╝██╔══██╗██╔════╝ ██╔══██╗████╗ ████║██╔════╝██╔══██╗${NC}"
    echo -e "${B_PURPLE}██║  ███╗██████╔╝██║  ███╗███████║██╔████╔██║█████╗  ██████╔╝${NC}"
    echo -e "${B_PURPLE}██║   ██║██╔══██╗██║   ██║██╔══██║██║╚██╔╝██║██╔══╝  ██╔══██╗${NC}"
...
while true; do
    ...
    read -r opt
    case $opt in
        1) bash <(curl -fsSL ${REPO}/vps/run.sh) ;;
        2) bash <(curl -fsSL ${REPO}/panel/run.sh) ;;
        3) bash <(curl -fsSL ${REPO}/wings/run.sh) ;;
        4) bash <(curl -fsSL ${REPO}/tools/run.sh) ;;
        5) bash <(curl -fsSL ${REPO}/theme/chang/dev.sh) ;;
        6) bash <(curl -fsSL ${REPO}/srv/menu/System1.sh) ;;
        7) bash <(curl -fsSL ${REPO}/no-kvm/run.sh) ;;
        8|exit) 
